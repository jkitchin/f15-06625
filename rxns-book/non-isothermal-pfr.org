...
colors = 'rgb'  # colors to make each solution
for i, Fi0 in enumerate([1.0, 0.1, 0.0]):
    ...
    plt.subplot(1,2,1)
    plt.plot(Vspan, X, colors[i], label='Fi0 = {0} mol/s'.format(Fi0))
    plt.xlabel('Volume (L)')
    plt.ylabel('Conversion')

    plt.subplot(1,2,2)
    plt.plot(Vspan, T, colors[i], label='Fi0 = {0} mol/s'.format(Fi0))
    plt.xlabel('Volume (L)')
    plt.ylabel('Temperature (K)')
...
Ta = 1150.0       # ambient heat exchanger temperature
...
plt.tight_layout()
plt.savefig('images/adiabatic-pfr-ambient-heat.png')
#+END_SRC

#+RESULTS:

[[./images/adiabatic-pfr-ambient-heat.png]]
...
plt.tight_layout()
plt.savefig('images/adiabatic-pfr-cocurrent-heat.png')
#+END_SRC

#+RESULTS:

[[./images/adiabatic-pfr-cocurrent-heat.png]]
...
Tshell0 = 1250.0    # ambient heat exchanger temperature
Ua = 0.0165         # overall heat transfer coefficient
mdot = 0.5          # mol/s coolant flow rate
Cpcoolant = 0.0345  # Heat capacity of coolant
...
Ta_guess = 1016.8  # for 0.5 mol/s coolant
# Ta_guess = 1133    # for 1 mol/s coolant
Y0 = [Fa0, T0, Ta_guess]
